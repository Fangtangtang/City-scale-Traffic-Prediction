...
class RLinear(nn.Module):
    def __init__(self, input_len:int, pred_len:int, drop_ratio:float) -> None:
        super(RLinear, self).__init__()

        self.Linear = nn.Linear(input_len, pred_len)
        
        self.dropout = nn.Dropout(drop_ratio)
        self.rev = RevIN(num_features=1)
...
for ii in idx[:]:
    ...
    input_len = 24
    pred_len = 1
    model = RLinear(input_len=input_len, pred_len=pred_len, drop_ratio=0.1).to(device)
